...
Architecture:
  Input (112x112 RGB)
       ...
       └── ViT-B/16 → 768-d Transformer features (optionally resized to vit_input_size)
            │
...
class ViTBackbone(nn.Module):
    ...
    def __init__(self, pretrained=True, img_size=112):
        super().__init__()
        ...
        self.vit = timm.create_model(
            ...
            img_size=img_size,   # NOTE: must match vit_input_size passed at inference
        )
...
class HybridFaceNet(nn.Module):
    ...
    Args:
        ...
        vit_input_size  : Image size fed to the ViT branch only (112 = original,
                          160 = improved — reduces position-embedding distortion).
                          ResNet always sees the image at its original size;
                          only the copy going into ViT gets resized.
    """
    ...
    def __init__(self, num_classes, fusion='concat', embed_dim=512,
        ...
        self.vit_input_size = vit_input_size   # image size fed to ViT only

        # Backbones
        self.cnn = ResNet50Backbone(pretrained=pretrained)
        self.vit = ViTBackbone(pretrained=pretrained, img_size=vit_input_size)
        ...
        print(f"[HybridFaceNet] fusion={fusion} | embed={embed_dim} | classes={num_classes} "
              f"| vit_input_size={vit_input_size}")

    def extract(self, x):
        """Returns L2-normalized embeddings (use for inference/evaluation)."""
        cnn_feat = self.cnn(x)   # ResNet sees the image at its original size (e.g. 112x112)

        # ViT gets a resized copy only — this does NOT change what ResNet sees,
        # and does NOT change anything in dataset.py / your dataloaders.
        if self.vit_input_size != x.shape[-1]:
...
class ResNet50ArcFace(nn.Module):
    ...
    def __init__(self, num_classes, embed_dim=512, pretrained=True,
                 arcface_scale=64.0, arcface_margin=0.5):
        super().__init__()
...
def build_model(model_type, num_classes, embed_dim=512, pretrained=True,
                 arcface_scale=64.0, arcface_margin=0.5, vit_input_size=112):
    """
    model_type: 'hybrid_concat' | 'hybrid_weighted' | 'hybrid_attention'
                'resnet50'      | 'vit'
    arcface_scale / arcface_margin: lower these for small-identity-count training
    vit_input_size: image size fed to the ViT branch only (112 = original, 160 = improved)
    """
...
if __name__ == "__main__":
    ...
    print("\nTesting with vit_input_size=160 (ViT gets bigger images than ResNet)...")
    m160 = HybridFaceNet(num_classes=100, fusion='concat', pretrained=False, vit_input_size=160)
    e160 = m160(x)   # x is still 112x112 — model resizes internally for ViT only
    print(f"  hybrid_concat @ vit_input_size=160: {e160.shape} ✓")

    print("\nTesting baselines with custom ArcFace settings...")
    rb = ResNet50ArcFace(num_classes=100, pretrained=False, arcface_scale=30.0, arcface_margin=0.3)
    print(f"  resnet50 baseline: {rb(x).shape} ✓")
    vb = ViTArcFace(num_classes=100, pretrained=False, arcface_scale=30.0, arcface_margin=0.3)
    print(f"  vit baseline: {vb(x).shape} ✓")