...
def load_lfw_pairs(pairs_file, img_dir, mismatch_file=None):
...
def main():
    ...
    if os.path.exists(args.lfw_dir) and os.path.exists(args.lfw_pairs):
        print("\nEvaluating on LFW...")
        pairs = load_lfw_pairs(args.lfw_pairs, args.lfw_dir)
        print(f"  {len(pairs)} pairs loaded")
        from torchvision import transforms as T; transform = T.Compose([T.Resize((160,160)), T.ToTensor(), T.Normalize([0.5],[0.5])]); m = evaluate_facenet(facenet, pairs, transform, device)
        results['lfw'] = m
